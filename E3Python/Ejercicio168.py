def cargar():
    contactos={}
    continua="s"
    while continua=="s":
        nombre=raw_input("Ingrese el nombre del contacto:")
        telefono=raw_input("Ingrese el numero de telefono:")
        contactos[nombre]=telefono
        continua=raw_input("Ingresa otro contacto[s/n]?:")
    return contactos


def modificar_telefono(contactos):
    nombre=raw_input("Ingrese el nombre de contacto a modificar el telefono:")
    if nombre in contactos:
        telefono=raw_input("Ingrese el nuevo numero telefonico")
        contactos[nombre]=telefono
    else:
        print("No existe un contacto con el nombre ingresado")


def imprimir(contactos):
    print("Listado de todos los contactos")
    for nombre in contactos:
        print(nombre,contactos[nombre])


# bloque principal

contactos=cargar()
modificar_telefono(contactos)
imprimir(contactos)